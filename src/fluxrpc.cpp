...
{
    return GetBoolArg("-testnet", false) ? 7978 : 7971;
}
...
int64 AmountFromValue(const Value& value)
{
    ...
    int64 nAmount = roundint64(dAmount * FLUX);
...
Value gettotalvolume(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "gettotalvolue\n"
            "Get the total amount of flux in existence.");
    // Shutdown will take long enough that the response should get back
...
{ //  name                      actor (function)         okSafeMode threadSafe
  //  ------------------------  -----------------------  ---------- ----------
    ...
    { "gettotalvolume",         &gettotalvolume,         true,      false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: Flux-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  Flux speaks version 1.0 for maximum compatibility,
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use Fluxd";
