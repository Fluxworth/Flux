...
#ifndef FLUX_WALLET_H
#define FLUX_WALLET_H
...
{
private:
    bool SelectFlux(int64 nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setFluxRet, int64& nValueRet) const;
...
public:
    ...
    std::set<COutPoint> setLockedFlux;

    // check whether we are allowed to upgrade (or already support) to the named feature
    bool CanSupportFeature(enum WalletFeature wf) { return nWalletMaxVersion >= wf; }

    void AvailableFlux(std::vector<COutput>& vFlux, bool fOnlyConfirmed=true) const;
    bool SelectFluxMinConf(int64 nTargetValue, int nConfMine, int nConfTheirs, std::vector<COutput> vFlux, std::set<std::pair<const CWalletTx*,unsigned int> >& setFluxRet, int64& nValueRet) const;
    bool IsLockedFlux(uint256 hash, unsigned int n) const;
    void LockFlux(COutPoint& output);
    void UnlockFlux(COutPoint& output);
    void UnlockAllFlux();
    void ListLockedFlux(std::vector<COutPoint>& vOutpts);
...
public:
    ...
    {
        // Must wait until Fluxbase is safely deep enough in the chain before valuing it
        if (IsFluxBase() && GetBlocksToMaturity() > 0)
            return 0;
    ...
    {
        if (IsFluxBase() && GetBlocksToMaturity() > 0 && IsInMainChain())
        {
