...
const int FLUX_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString FLUX_IPC_PREFIX("Flux:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(FLUX_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(FLUX_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(FLUX_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click Flux: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start Flux: click-to-pay handler");
    else
...
{
    // clicking on Flux: URLs creates FileOpen events on the Mac:
