...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("GLN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("flux:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            ret += QString("?amount=%1").arg(FluxUnits::format(FluxUnits::FLX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
