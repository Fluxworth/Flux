    ...
    model(0)
{
    ...
    ui->addressIn_SM->setPlaceholderText(tr("Enter a Flux address (e.g. mhN8btKtp3HrXgZJwyBakUzALLZ34nA4J)"));
    ui->pubkeyOut_VM->setPlaceholderText(tr("Click \"Sign Message\" to view public key"));
    ui->signatureOut_SM->setPlaceholderText(tr("Click \"Sign Message\" to generate signature"));

    ui->addressIn_VM->setPlaceholderText(tr("Enter a Flux address (e.g. mhN8btKtp3HrXgZJwyBakUzALLZ34nA4J)"));
    ui->pubkeyIn_VM->setPlaceholderText(tr("Enter the public key used for signing"));
    ui->signatureIn_VM->setPlaceholderText(tr("Enter Flux signature"));
#endif
    ...
    ui->signatureOut_SM->setFont(GUIUtil::fluxAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::fluxAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CFluxAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid())
    {
...
{
    CFluxAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CFluxAddress(key.GetPubKey().GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
